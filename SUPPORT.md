...
Net Lattice has landed Stage 0.10 of its [architecture](ARCHITECTURE.md)'s Incremental Delivery Plan (route and interface-address mutation, interface, DNS-read, neighbor-read, address-read, and bounded/filterable monitoring providers for Linux, Windows, and macOS). Usage support is limited to that surface for now — see [README.md](README.md)'s Current Status — but questions about usage, direction, and design are all welcome.
