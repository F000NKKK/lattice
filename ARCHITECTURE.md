...
| 0.15 ◐ | Transaction execution baseline: runtime capability preflight via `Lattice::validate_plan`, ordered submission through `Lattice::execute_plan`, per-operation outcomes, first-failure stopping, operation-boundary cancellation, and an explicitly supplied reverse-order compensator. Native snapshot capture and cross-platform privileged integration tests remain to be completed. |
