...
| 0.15 ◐ | Transaction execution baseline: `Lattice::execute_plan` submits ordered plans, records per-operation outcomes, stops at the first failure, and supports operation-boundary cancellation. Native compensation with prior-state snapshots and cross-platform privileged integration tests remain to be completed. |
