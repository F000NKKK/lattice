...
| 0.15 ◐ | Transaction execution baseline: `Lattice::execute_plan` submits ordered plans, records per-operation outcomes, stops at the first failure, supports operation-boundary cancellation, and `execute_plan_with_compensation` runs an explicitly supplied reverse-order compensator. Native snapshot capture and cross-platform privileged integration tests remain to be completed. |
