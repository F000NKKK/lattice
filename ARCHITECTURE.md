...
| 0.15 ◐ | Transaction execution baseline: runtime capability preflight via `Lattice::validate_plan`, ordered submission through `Lattice::execute_plan`, per-operation outcomes, first-failure stopping, operation-boundary cancellation, caller-defined prior-state capture via `execute_plan_with_snapshot`, and an explicitly supplied reverse-order compensator. Native platform snapshots and cross-platform privileged integration tests remain to be completed. |
