...
Net Lattice has landed Stage 0.1 and Stage 0.2 of its [architecture](ARCHITECTURE.md)'s Incremental Delivery Plan — minimal Linux and Windows route providers, not a complete library. Most public API surface is still unbuilt. The most valuable contributions right now are:
...
- Implementation work on the next stages in [ARCHITECTURE.md](ARCHITECTURE.md)'s delivery plan (BSD/macOS backends, `interface`/`dns`/`neighbor` modules, ...)
