...
> **Статус:** В репозитории реализованы этапы 0.1 и 0.2 плана архитектуры. На текущий момент присутствует несколько крейтов с реальной реализацией для просмотра, добавления и удаления маршрутов IPv4/IPv6 в Linux и Windows. Это минимальный законченный вертикальный срез, а не полноценная библиотека — см. раздел «Текущий статус» ниже.
...
Это даёт реальное, опубликованное управление маршрутами на Linux и Windows. Это всё ещё не полноценная библиотека: интерфейсы, DNS, соседи, BSD/macOS и все остальные пункты из долгосрочных целей выше ещё впереди; см. [ARCHITECTURE.ru.md](ARCHITECTURE.ru.md) для поэтапной дорожной карты и [CHANGELOG.md](CHANGELOG.md) для того, что реально вышло.
