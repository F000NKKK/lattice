...
> **Статус:** В репозитории реализованы этапы 0.1, 0.2 и 0.3 плана архитектуры. На текущий момент присутствует несколько крейтов с реальной реализацией для просмотра, добавления и удаления маршрутов IPv4/IPv6 в Linux, Windows и BSD/macOS. Это минимальный законченный вертикальный срез, а не полноценная библиотека — см. раздел «Текущий статус» ниже.
...
- `net-lattice-backend-darwin` (маршруты через BSD/macOS route sockets)
- фасад `net-lattice`

Это даёт реальное, опубликованное управление маршрутами на Linux, Windows и BSD/macOS. Это всё ещё не полноценная библиотека: интерфейсы, DNS, соседи и все остальные пункты из долгосрочных целей выше ещё впереди; см. [ARCHITECTURE.ru.md](ARCHITECTURE.ru.md) для поэтапной дорожной карты и [CHANGELOG.md](CHANGELOG.md) для того, что реально вышло.
...
4. **Паритет платформ** *(завершён для маршрутов)* — shipped Linux/Windows/BSD/macOS бэкенды для маршрутов.
