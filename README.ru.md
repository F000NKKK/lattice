...
> **Статус:** В репозитории реализован этап 0.1 плана архитектуры. На текущий момент присутствует несколько крейтов с реальной реализацией, сосредоточенных на просмотре, добавлении и удалении маршрутов IPv4/IPv6 в Linux. Это минимальный законченный вертикальный срез, а не полноценная библиотека — см. раздел «Текущий статус» ниже.
...
Реализован этап 0.1 плана поэтапной поставки из [архитектуры](ARCHITECTURE.ru.md):
...
Это минимальный сквозной срез — просмотр, добавление и удаление IPv4/IPv6-маршрутов в Linux, — а не полноценная библиотека. Интерфейсы, DNS, соседи, другие платформы и все остальные пункты из долгосрочных целей выше ещё впереди; см. [ARCHITECTURE.ru.md](ARCHITECTURE.ru.md) для поэтапной дорожной карты и [CHANGELOG.md](CHANGELOG.md) для того, что реально вышло.

## Дорожная карта

1. **Bootstrap** *(завершён)* — инфраструктура репозитория, лицензирование, файлы для сообщества и настройка инструментов.
2. **Проектирование** *(завершён)* — определение структуры крейтов, базовых абстракций и стратегии абстрагирования платформ. См. [ARCHITECTURE.ru.md](ARCHITECTURE.ru.md) для планируемой структуры workspace.
3. **Фундамент** *(текущий этап)* — реализация базовых типов (адреса, префиксы, интерфейсы) и первого платформенного бэкенда.
