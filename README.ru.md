...
> **Статус:** В репозитории реализованы этапы 0.1, 0.2 и 0.3 плана архитектуры. На текущий момент присутствует несколько крейтов с реальной реализацией для просмотра, добавления и удаления маршрутов IPv4/IPv6 в Linux и Windows, а также каркас BSD/macOS бэкенда. Это минимальный законченный вертикальный срез, а не полноценная библиотека — см. раздел «Текущий статус» ниже.
...
Реализованы этапы 0.1, 0.2 и 0.3 плана поэтапной поставки из [архитектуры](ARCHITECTURE.ru.md):

- `net-lattice-core`, `net-lattice-ip`
- модуль `route` в `net-lattice-model`
- `RouteProvider` в `net-lattice-platform`
- `net-lattice-backend-linux` (маршруты через Netlink)
- `net-lattice-backend-windows` (маршруты через Windows IP Helper API)
- `net-lattice-backend-darwin` (каркас BSD/macOS route socket бэкенда)
- фасад `net-lattice`

Это даёт реальное, опубликованное управление маршрутами на Linux и Windows, а также каркас BSD/macOS бэкенда. Это всё ещё не полноценная библиотека: интерфейсы, DNS, соседи и все остальные пункты из долгосрочных целей выше ещё впереди; см. [ARCHITECTURE.ru.md](ARCHITECTURE.ru.md) для поэтапной дорожной карты и [CHANGELOG.md](CHANGELOG.md) для того, что реально вышло.
...
3. **Фундамент** *(завершено для маршрутов)* — реализованы базовые типы IP/маршрутов и бэкенды Linux/Windows. Осталось: интерфейсы, DNS, соседи.
4. **Паритет платформ** *(в процессе)* — shipped Linux/Windows бэкенды для маршрутов; BSD/macOS бэкенд добавлен как каркас и требует реализации.
