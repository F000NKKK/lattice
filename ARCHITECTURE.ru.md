...
| 0.15 ◐ | Базовое исполнение транзакций: runtime capability preflight через `Lattice::validate_plan`, provider-backed capture `MutationSnapshot` через `snapshot_for_mutation`, отправка операций по порядку через `Lattice::execute_plan`, результат каждой операции, остановка после первой ошибки, cancellation на границе операции, capture prior state через `execute_plan_with_snapshot` и явно переданный compensator в обратном порядке. Native platform integration tests для всех платформ ещё предстоит завершить. |
