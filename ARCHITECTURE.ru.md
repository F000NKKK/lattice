...
| 0.15 ◐ | Базовое исполнение транзакций: runtime capability preflight через `Lattice::validate_plan`, отправка операций по порядку через `Lattice::execute_plan`, результат каждой операции, остановка после первой ошибки, cancellation на границе операции, capture prior state через `execute_plan_with_snapshot` и явно переданный compensator в обратном порядке. Native platform snapshots и privileged integration tests для всех платформ ещё предстоит завершить. |
