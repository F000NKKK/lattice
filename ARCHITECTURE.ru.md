...
теперь существуют начиная с этапа 0.19. `ApplyPlan` пока не существует —
он относится к этапу 0.20, после стабилизации transaction-контрактов и
оставшейся imperative mutation-поверхности, когда можно будет исполнять
diff. Разделение state/config названо здесь — как параллельный тип `*Config` на
...
Строки до 0.19 включительно реализованы и доступны сегодня; строки начиная с
0.20 описывают планируемую, ещё не реализованную работу:
