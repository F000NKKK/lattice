...
| 0.15 ◐ | Базовое исполнение транзакций: `Lattice::execute_plan` отправляет операции плана по порядку, сохраняет результат каждой операции, останавливается после первой ошибки, поддерживает cancellation на границе операции, а `execute_plan_with_compensation` запускает явно переданный compensator в обратном порядке. Native snapshot capture и privileged integration tests для всех платформ ещё предстоит завершить. |
