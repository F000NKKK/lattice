...
| 0.15 ◐ | Базовое исполнение транзакций: `Lattice::execute_plan` отправляет операции плана по порядку, сохраняет результат каждой операции, останавливается после первой ошибки и поддерживает cancellation на границе операции. Native compensation с prior-state snapshots и privileged integration tests для всех платформ ещё предстоит завершить. |
