...
| 0.15 ◐ | Базовое исполнение транзакций: runtime capability preflight через `Lattice::validate_plan`, отправка операций по порядку через `Lattice::execute_plan`, результат каждой операции, остановка после первой ошибки, cancellation на границе операции и явно переданный compensator в обратном порядке. Native snapshot capture и privileged integration tests для всех платформ ещё предстоит завершить. |
