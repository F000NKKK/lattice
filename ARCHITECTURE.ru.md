...
`DesiredState` (форма данных, в `net-lattice-model`, см. выше) теперь
существует начиная с этапа 0.19. `Diff` и `ApplyPlan` пока не существуют —
они относятся к оставшейся части этапа 0.19 и к этапу 0.20, после
стабилизации transaction-контрактов и оставшейся imperative
